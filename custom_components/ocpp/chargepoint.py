...
class ChargePoint(cp):
    ...
    @staticmethod
    def fix_malformed_json(msg: str) -> str:
        """Fix known malformed JSON patterns in OCPP messages."""
        # Fix empty configurationKey array syntax
        if '"configurationKey":]' in msg:
            msg = msg.replace('"configurationKey":]', '"configurationKey":[]')
        return msg
    ...
    async def route_message(self, raw_msg):
        """
        Override the route_message method to handle malformed JSON messages.
        """
        try:
            # Try to fix any known malformed patterns
            fixed_msg = self.fix_malformed_json(raw_msg)  # <- Correction ici, ajout de self.
            
            # If message was fixed, log it
            if fixed_msg != raw_msg:
                _LOGGER.debug(
                    f"Fixed malformed message for {self.id}. Original: {raw_msg}, Fixed: {fixed_msg}"
                )
                raw_msg = fixed_msg
            ...
            _LOGGER.error(
                f"Error processing message for {self.id}: {str(e)}, Message: {raw_msg}"
            )
            raise
            ...
            for idx in range(len(bucket)):
                ...
                if phase is None:
                    if (
                        ...
                        if is_transaction:
                            session_value = max(0, value)  # Ajouter cette ligne
                            self._metrics[csess.session_energy.value].value = session_value
                        ...
                        if (
                            ...
                            session_energy = round(1000 * (value - meter_start)) / 1000
                            self._metrics[csess.session_energy.value].value = max(0, session_energy)
                            self._metrics[csess.session_energy.value].unit = unit
