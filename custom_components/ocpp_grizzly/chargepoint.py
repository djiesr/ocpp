...
class ChargePoint(cp):
    ...
    async def _handle_call(self, msg):
        """Handle incoming calls with JSON malformation fixes."""
        try:
            # If msg is already a Call object, use it directly
            if hasattr(msg, 'to_json'):
                ...
                try:
                    ...
                    # If JSON parsing fails, try to fix known malformations
                    fixed_msg = self.fix_malformed_json(msg)
                    try:
                        # Try again with fixed message
                        json.loads(fixed_msg)
                        await super()._handle_call(fixed_msg)
                    except (json.JSONDecodeError, FormatViolationError) as e:
                        # If still fails, create and send error response
